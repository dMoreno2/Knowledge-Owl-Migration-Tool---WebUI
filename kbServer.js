...
var port = Math.floor(Math.random() * (8000 - 5000 + 1)) + 5000;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET") {
      ...
      if (req.url === "/update&Create") {
        ...
        const reqParams = req.url.split("/");
        const inputValue = reqParams[2];
        Output(`Creating article ${inputValue}`);
        res.end();
...
function Output(outputData) {
  ...
  log_to_server_queue.push(outputData);
}
...
async function LogData() {
  while (true) {
    ...
    if (log_to_file_queue.length > 0) {
      await fs.appendFile(
        logFilePath,
        `"${GetDateTime()}":${JSON.stringify(log_to_file_queue[0], null, 2)}\n`,
        (err) => { }
      );
